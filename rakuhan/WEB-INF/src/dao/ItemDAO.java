	...
	public ArrayList<Item> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				item.setUpdated_at(rs.getDate("updated_at"));
...
//	該当ユーザーの出品情報取得(出品済み商品一覧機能)
	public ArrayList<Item> selectByUser(User user) {
		Connection con = null;
		Statement smt = null;

		ArrayList<Item> list = new ArrayList<Item>();
		try {
			String sql = "select * from iteminfo where userid = '" + user.getUserid() + "'";
	...
	public void update(Item item) {
		...
		try {
			String sql = "update iteminfo set itemname = '" + item.getItemname() + "', type = '" + item.getType()
					...
					+ item.getStatus() + "', updated_at= ' " + item.getUpdated_at() + "' where itemid  = "
					+ item.getItemid();
