...
public class LoginServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request ,HttpServletResponse response) throws ServletException ,IOException
	{
		try{
			...
			//DBから該当ユーザ情報を検索
			user = user.selectByUser(userid,pass);
			...
			if(user.getUser() == null){
				...
				//クッキーにユーザ名を保存
				Cookie cookieName = new Cookie("userid",URLEncoder.encode(user.getUser(),"UTF-8"));
				...
				//クッキーにパスワードを保存
				Cookie cookiePass = new Cookie("pass",URLEncoder.encode(pass.getPassword(),"UTF-8"));
